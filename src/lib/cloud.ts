...
export async function loadCloudStudio(uid: string): Promise<StudioData | null> {
  ...
  return {
    ...
    moods: hydrateMoodBook(raw.moods, defaultMoodBook()),
    productivity: hydrateMoodBook(raw.productivity, defaultProductivityBook()),
  }
}
...
export async function saveCloudStudio(uid: string, data: StudioData) {
  ...
  await setDoc(doc(db, 'users', uid, 'data', 'studio'), {
    ...
    moods: hydrateMoodBook(data.moods, defaultMoodBook()),
    productivity: hydrateMoodBook(data.productivity, defaultProductivityBook()),
    updatedAt: Date.now(),
  })
}
